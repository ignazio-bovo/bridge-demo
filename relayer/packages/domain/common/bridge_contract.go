...
func GetContractABI() (*abi.ABI, error) {
	bridge, err := abi.JSON(strings.NewReader(CONTRACT_ABI))
	if err != nil {
		return nil, fmt.Errorf("fail to unmarshal erc20 abi: %w", err)
	}
	return &bridge, nil
}

func GetTransferRequestedSignature() common.Hash {
	signatureString := "TransferRequested((uint256,address,address,bytes32,uint256,uint64,uint64))"
	return crypto.Keccak256Hash([]byte(signatureString))
}
