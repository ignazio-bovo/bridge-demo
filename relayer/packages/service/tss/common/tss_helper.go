...
func ConvertMsgToBigInt(msg []byte) (*big.Int, error) {
	z := new(big.Int)
	z.SetBytes(msg)
	return z, nil
}
...
func verifySignature(pubKey crypto.PubKey, message, sig []byte, msgID string) bool {
	...
	ok, err := pubKey.Verify(dataForSign.Bytes(), sig)
	if err != nil {
		log.Error().Msgf("fail to verify the signature: %s", err)
		return false
	}
	return ok
}
...
func GetMsgRound(msg []byte, partyID *btss.PartyID, isBroadcast bool, handleID uint64) (blame.RoundInfo, error) {
	parsedMsg, err := btss.ParseWireMessage(msg, partyID, isBroadcast)
	if err != nil {
		return blame.RoundInfo{}, err
	}

	_, err = conversion.GetPeerIDFromPartyID(partyID)
	if err != nil {
		return blame.RoundInfo{}, err
	}
	...
	case *resharing.DGRound1Message:
		return blame.RoundInfo{
			Index:    0,
			RoundMsg: messages.KEYRESHARING1,
		}, nil

	case *resharing.DGRound2Message1:
		return blame.RoundInfo{
			Index:    1,
			RoundMsg: messages.KEYRESHARING2aUnicast,
		}, nil

	case *resharing.DGRound2Message2:
		return blame.RoundInfo{
			Index:    2,
			RoundMsg: messages.KEYRESHARING2b,
		}, nil

	case *resharing.DGRound3Message1:
		return blame.RoundInfo{
			Index:    3,
			RoundMsg: messages.KEYRESHARING3a,
		}, nil

	case *resharing.DGRound3Message2:
		return blame.RoundInfo{
			Index:    4,
			RoundMsg: messages.KEYRESHARING3b,
		}, nil

	case *resharing.DGRound4Message:
		return blame.RoundInfo{
			Index:    5,
			RoundMsg: messages.KEYRESHARING4,
		}, nil
	default:
		return blame.RoundInfo{}, fmt.Errorf("unknown round")
	}
}

func (t *TssCommon) NotifyTaskDone(msgID string) error {
...
func (t *TssCommon) processRequestMsgFromPeer(peersID []peer.ID, msg *messages.TssControl, requester bool, msgID string, handleID uint64) error {
...
func GeneratePreParams(timeout time.Duration) (*bkeygen.LocalPreParams, error) {
	...
	for i := 0; i < retryCount; i++ {
		...
		log.Warn().Msgf("Failed to generate pre parameters, attempt %d/%d: %v", i+1, retryCount, err)
		time.Sleep(2 * time.Second)
	}
	if err != nil {
		return nil, fmt.Errorf("fail to generate pre parameters after %d attempts: %w", retryCount, err)
	}
