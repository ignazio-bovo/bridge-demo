...
type signatureItem struct {
	messageID     string
	peerID        peer.ID
	signatureData []*tsslibcommon.SignatureData
}
...
type SignatureNotifier struct {
	...
	signatureNotifierProtocol protocol.ID
}

// NewSignatureNotifier create a new instance of SignatureNotifier
func NewSignatureNotifier(host host.Host, handlerID uint64) *SignatureNotifier {
	s := &SignatureNotifier{
		...
		signatureNotifierProtocol: protocol.ID("/p2p/signatureNotifier" + fmt.Sprintf("%d", handlerID)),
	}

	host.SetStreamHandler(s.signatureNotifierProtocol, s.handleStream)
	return s
}

func (s *SignatureNotifier) RemoveStreamHandler() bool {
	s.host.RemoveStreamHandler(s.signatureNotifierProtocol)
	return true
}
...
func (s *SignatureNotifier) sendOneMsgToPeer(m *signatureItem) error {
	...
	stream, err := s.host.NewStream(ctx, m.peerID, s.signatureNotifierProtocol)
	if err != nil {
