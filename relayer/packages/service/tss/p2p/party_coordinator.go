...
import (
	...
	messages "github.com/Datura-ai/relayer/packages/service/tss/messages"
...
type PartyCoordinator struct {
	...
	keysignChan        *chan common.BridgeTxData
...
func (pc *PartyCoordinator) GetKeyGenChan() *chan []peer.ID {
	return pc.keygenChan
}
...
func (pc *PartyCoordinator) processRespMsg(respMsg *messages.JoinPartyLeaderComm, stream network.Stream) {
	...
	if !ok {
		switch respMsg.RequestType {
		case string(domain.KEYSIGN_MSG_TYPE):
			pc.logger.Info().Msg("this party is not ready for keysign")
			// //TODO: need to trigger the keysign process here to msgChan
			peerStatus, err := pc.processKeySignMsg(respMsg)
			if err != nil {
				pc.logger.Error().Err(err).Msg("fail to create join party groups")
				return
			}

			peerGroup = peerStatus
			//*pc.keysignChan <- txData
		default:
...
func (pc *PartyCoordinator) processReqMsg(requestMsg *messages.JoinPartyLeaderComm, stream network.Stream) {
	...
	if !ok {
		...
		case string(domain.KEYGEN_MSG_TYPE):
			pc.logger.Info().Msg("this party is not ready for keygen")
			peerIDs, err := pc.getPeerIDs(requestMsg.PeerIDs)
			if err != nil {
			...
			*pc.keygenChan <- peerIDs
		case string(domain.KEYSIGN_MSG_TYPE):
			// pc.logger.Info().Msg("this party is not ready for keysign")
			// //TODO: need to trigger the keysign process here to msgChan
			// txData := common.BridgeTxData{}
			// err := txData.FromBytes(requestMsg.MsgPayload)
			// if err != nil {
			// 	pc.logger.Error().Err(err).Msg("fail to parse the tx data")
			// 	return
			// }
			// *pc.keysignChan <- txData
			pc.logger.Info().Msg("this party is not ready for keysign")
			...
			// 	return
			// }
			// leaderID, err := peer.Decode(requestMsg.LeaderNodeID)
			// if err != nil {
			// 	pc.logger.Error().Err(err).Msg("fail to decode leader id")
			// 	return
			// }
			// peers, err := pc.getPeerIDs(requestMsg.PeerIDs)
			// if err != nil {
			// 	pc.logger.Error().Err(err).Msg("fail to get peer ids")
			// 	return
			// }
			// threshold := int(requestMsg.Threshold)
			// peerStatus, err := pc.createJoinPartyGroups(requestMsg.ID, leaderID, peers, threshold)
			// if err != nil {
			// 	pc.logger.Error().Err(err).Msg("fail to create join party groups")
			// 	return
			// }
			// pc.peersGroup[requestMsg.ID] = peerStatus
			newPeerStatus, err := pc.processKeySignMsg(requestMsg)
			if err != nil {
				pc.logger.Error().Err(err).Msg("fail to create join party groups")
				return
			}
			peerGroup = newPeerStatus
		default:
			pc.logger.Info().Msg("this party is not ready")
			_ = stream.Reset()
		}
...
func (pc *PartyCoordinator) processKeySignMsg(msg *messages.JoinPartyLeaderComm) (*peerStatus, error) {

	txData := common.BridgeTxData{}
	err := txData.FromBytes(msg.MsgPayload)
	if err != nil {
		pc.logger.Error().Err(err).Msg("fail to parse the tx data")
		return nil, err
	}
	leaderID, err := peer.Decode(msg.LeaderNodeID)
	if err != nil {
		pc.logger.Error().Err(err).Msg("fail to decode leader id")
		return nil, err
	}
	peers, err := pc.getPeerIDs(msg.PeerIDs)
	if err != nil {
		pc.logger.Error().Err(err).Msg("fail to get peer ids")
		return nil, err
	}
	threshold := int(msg.Threshold)
	peerStatus, err := pc.createJoinPartyGroups(msg.ID, leaderID, peers, threshold)
	if err != nil {
		pc.logger.Error().Err(err).Msg("fail to create join party groups")
		return nil, err
	}
	pc.peersGroup[msg.ID] = peerStatus
...
func (pc *PartyCoordinator) joinPartyMember(msgID, requestType string, peerGroup *peerStatus, payload []byte, sigChan chan string) ([]peer.ID, error) {
	leaderID := peerGroup.getLeader()
	msg := messages.JoinPartyLeaderComm{
		ID:           msgID,
		RequestType:  requestType,
		LeaderNodeID: leaderID.String(),
		Threshold:    int32(peerGroup.threshold),
		PeerIDs:      convertPeerIDsToStrings(peerGroup.getAllPeers()),
		MsgPayload:   payload,
	}
	...
	if peerGroup.getLeaderResponse() == nil {
		leaderPk, err := leaderID.ExtractPublicKey()
		if err != nil {
...
func (pc *PartyCoordinator) joinPartyLeader(msgID string, requestType string, peerGroup *peerStatus, payload []byte, sigChan chan string) ([]peer.ID, error) {
	var sigNotify string
	select {
	case <-pc.stopChan:
		// promptly tear down this goroutine if partyCoordinator is stopped
	...
	msg := messages.JoinPartyLeaderComm{
		...
		RequestType:  requestType,
		LeaderNodeID: peerGroup.leader.String(),
		Threshold:    int32(peerGroup.threshold),
		PeerIDs:      tssNodes,
		MsgPayload:   payload,
	}
...
func (pc *PartyCoordinator) JoinPartyWithLeader(msgID, requestType string, blockHeight int64, peers []string, threshold int, payload []byte, sigChan chan string) ([]peer.ID, string, error) {
	...
	if pc.host.ID() == leaderID {
		onlines, err := pc.joinPartyLeader(msgID, requestType, peerGroup, payload, sigChan)
		return onlines, leader, err
	}
	// now we are just the normal peer
	onlines, err := pc.joinPartyMember(msgID, requestType, peerGroup, payload, sigChan)
	return onlines, leader, err
}

func (pc *PartyCoordinator) IsLeader(msgID string, blockHeight int64, peers []string) (bool, error) {
	leader, err := LeaderNode(msgID, blockHeight, peers)
	if err != nil {
		return false, err
	}
	...
	return pc.host.ID() == leaderID, nil
}
...
func (pc *PartyCoordinator) SetNotifiers(keyGenChan *chan []peer.ID, keysignChan *chan common.BridgeTxData) {
	pc.keygenChan = keyGenChan
	pc.keysignChan = keysignChan
}

// Helper function to convert []peer.ID to []string
func convertPeerIDsToStrings(peerIDs []peer.ID) []string {
	peerStrings := make([]string, len(peerIDs))
	for i, pid := range peerIDs {
		peerStrings[i] = pid.String()
	}
	return peerStrings
}
